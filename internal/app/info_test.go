...
func TestEnvInfo(t *testing.T) {
	...
	store.Clear()
	var buf bytes.Buffer
	ok, err := app.Info(&buf, "vars", []string{})
	if !ok || err != nil {
		t.Errorf("invalid error: %v", err)
	}
	if buf.String() != "\n" {
	...
	buf = bytes.Buffer{}
	ok, err = app.Info(&buf, "vars", []string{"garbage"})
	if !ok || err != nil {
		t.Errorf("invalid error: %v", err)
	}
	if buf.String() != "\n" {
	...
	if _, err = app.Info(&buf, "vars", []string{"test", "default"}); err.Error() != "invalid env command, too many arguments" {
		t.Errorf("invalid error: %v", err)
	}
}
