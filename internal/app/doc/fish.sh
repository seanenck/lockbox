...
function {{ $profile.Name }}
  ...
  complete -c {{ $.Executable }} -n "__fish_seen_subcommand_from {{ $.HelpCommand }}" -a "{{ $.HelpAdvancedCommand }}"
...
{{- if $profile.CanTOTP }}
  ...
  complete -c {{ $.Executable }} -n "__fish_seen_subcommand_from {{ $.TOTPCommand }}" -a "$totps"
{{- if $profile.CanList }}
